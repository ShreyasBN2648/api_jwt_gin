...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			msg := fmt.Sprintf("Created time could not be updated.")
		...
		if err != nil {
			msg := fmt.Sprintf("Cannot update the updated time.")
			c.JSON(http.StatusInternalServerError, gin.H{"Error": msg})
			return
		}
		...
		token1, refreshToken, err := helpers.GenerateAllTokens(user.Email, user.First_Name, user.Last_Name, user.User_Type, user.User_ID)
		if err != nil {
			log.Panic(err)
		}
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err1 := userCollection.FindOne(ctx, bson.M{"email": user.Email}).Decode(&foundUser)
		if err1 != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"Error": "Email or password entered is incorrect."})
			return
		}
		...
		if foundUser.Email == "" || foundUser.Password == "" {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Please enter a valid email or password."})
		}
...
func verifyPassword(userPassword string, foundUserPassword string) (bool, string) {
	...
	if err != nil {
		msg = fmt.Sprintf("Email or password does not match.")
		check = false
	}
	return check, msg
}
...
func GetUsers() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var allUsers []bson.M
		if err2 := result.All(ctx, &allUsers); err != nil {
			log.Fatal(err2)
		}
